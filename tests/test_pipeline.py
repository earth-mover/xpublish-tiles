...
@pytest.mark.asyncio
@pytest.mark.parametrize("var_name", ["foo", "ua"])
async def test_pipeline_fvcom_ugrid_polygons(var_name, png_snapshot, pytestconfig):
    """Node (``foo``) and face (``ua``) variables of the real FVCOM mesh as polygons."""
    ds = FVCOM_MACHIAS_BAY.create()
    tile = Tile(x=160, y=184, z=9)
    query_params = replace(
        create_query_params(tile, WEBMERC_TMS, style="polygons"), variables=[var_name]
    )
    result = await pipeline(ds, query_params)
    if pytestconfig.getoption("--visualize"):
        visualize_tile(result, tile)
