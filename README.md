...
10. `XPUBLISH_TILES_VECTOR_MAX_FEATURES_PER_SIDE: int` - per-axis upper bound on the number of polygon features emitted per vector tile (default: `512`). The `vector` style coarsens dense input grids down to this target before encoding, capped further by `XPUBLISH_TILES_MAX_NUM_GEOMETRIES`. Lower it (e.g. 256) for smaller tiles at low zoom, raise it (e.g. 1024) for crisper low-zoom views of high-resolution datasets at the cost of larger tile bodies.
